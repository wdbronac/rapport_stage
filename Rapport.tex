...
\[ \lambda_i = N_0 e^{\int{\Delta}{} -\mub(s) ds}\]  
...
On peut cependant discrétiser cette loi en considérant une matrice de projection $\Pv$, qui appliquée à $\mub$ donnera un vecteur $\lambdab$ dont chaque coefficient $\lambdab_i$ approche très raisonnablement l'intégrale $N_0 \int_{\Delta_i}{} -\mub(s) ds$. La formule précédente devient alors:
...
\subsection{Paramètres propres à une tomographie}On distinguera les paramètres structurels du tomographe, et les paramètres variables, qu'un médecin est susceptible d'ajuster lors de chaque scan.
...
\item[Distance centre-détecteurs:] idem
			...
			\item 
De plus, on fait un calcul qui va faire en sorte d'ajouter des tranches de manière à ce que le faisceau de rayons croise totallement toutes les tranches lors de la projection, et l'on soustraira les tranches ajoutées dans la reconstruction. Voir figure \ref{fig:slices} pour plus de détails.
\end{description}

\subsubsection{Paramètres variables}
\begin{description}
\item[mAs:] Il s'agit de l'intensité de courant injectée dans chaque détecteur: elle est proportionnelle au compte de photons par seconde et par $cm^2$ arrivant sur le détecteur.
\item[]
\end{description}
...
On génère 3*2 images pour une échelle variant de X à X (zone de variation sensible des résultats), et pour trouver le lambda identique entre polaires et cartésiennes on regarde à l'oeil nu les images 2 à 2 et on essaye de faire qu'elles soient le plus identiques possibles. On utilise ici le critère d'adéquation Gaussien en guise d'exemple car c'est le plus simple, et la pénalisation GradL2 uniquement car on étudiera GradL2L1 dans la partie suivante.
\paragraph{Résultats}
\begin{figure}[H]
  \centering
  ...
  \hspace{5pt}
  \label{fig:resultats}
  \subfloat[Cylindrical: $\lambda=1e{-1}$]{\label{fig:contour-c}\includegraphics[scale=0.35]{cyl0p1.eps}}
\\
  \subfloat[Cartésiennes: $\lambda=1$]{\label{fig:contour-b}\includegraphics[scale=0.35]{cart1.eps}}
  \hspace{5pt}
  \subfloat[Cylindrical: $\lambda=1e{-2}$]{\label{fig:contour-c}\includegraphics[scale=0.35]{cyl0p01.eps}}
\\
  \centering
  \subfloat[Cartésiennes: $\lambda=1e{-1}$]{\label{fig:contour-b}\includegraphics[scale=0.35]{cart0p1.eps}}
  \hspace{5pt}
  \subfloat[Cylindrical: $\lambda=1e{-3}$]{\label{fig:contour-c}\includegraphics[scale=0.35]{cyl0p001.eps}}
  \caption{Image associations for which the $\lambda$ that has the same effect, for \texttt{PenalGrad\_L2}.}
\end{figure}
\paragraph{Analyse} 
On voit que plus $\lambda$ augmente, plus les hautes fréquences de l'image sont détruites. L'on voit aussi qu'une valeur de 1 (resp.$1e-2$) en coordonnées cartésiennes (resp. polaires) est une bonne valeur pour obtenir une image ni floue ni bruitée. Nous résumons les résultats dans le tableau suivant: \ref{table:nonlin}:
\begin{table}[H]
\label{table:nonlin}
\caption{Results sum-up}
\centering
...
\begin{figure}[H]
  ...
  \hspace{5pt}
  \label{fig:resultats}
  \subfloat[Cylindrical: $\lambda=1e{-3}$, \texttt{PenalGrad\_L2L1}]{\label{fig:contour-c}\includegraphics[scale=0.20]{cyl1e-3l1l2.eps}}
\\
  \centering
  \subfloat[Cartesian: $\lambda=1e{-1}$, \texttt{PenalGrad\_L2}]{\label{fig:contour-b}\includegraphics[scale=0.35]{cart0p1.eps}}
  \hspace{5pt}
  \subfloat[Cylindrical: $\lambda=1e{-3}$, \texttt{PenalGrad\_L2}]{\label{fig:contour-c}\includegraphics[scale=0.35]{cyl0p001.eps}}
  \caption{Image associations for which the $\lambda$ that has the same effect}
\end{figure}
...
\label{Parametres}
\begin{itemize}
\item Images générées: fantômes avec deux boules de fer de diamètre 2 cm et deux boules d'aluminium de rayon 6 cm (voir schéma).
...
\end{figure}
%%%%%%  %%%%%          %%%%%%%			%%%%%%
\subsubsection{Résultats: comparaison entre un seul et plusieurs rayons par détecteur}
%figure%%%%%%%%%%%
\begin{figure}[H]
  \centering
...
\begin{itemize}
\item Ini: Intensité de l'image d'initialisation 
\item Iter: Nombre d'itérations 
...
\end{tabular}
\end{table}
\subsection{Robustesse face au bruit: comparaison Gaussien/Poissonnien}
...
Ini      & Iter    & f(x)fin  & Rel.desc. & |Pg(x)|  &Coût & Raison de \\ 
          &          &            &                  &            & 	       &la convergence \\[0.5ex] 
\hline
$0$ &  286  &     1.28e4    		       &         9.15e-4   & 1.39e-6      & 1 & tol \\\\
$20$ &  28  &      1.28e4     		       &  0.00532 & 6.28e-3  & 1   & tol \\ [1ex]
\hline
\end{tabular}
\end{table}
%tableau-------------------------------
...
\begin{figure}[H]
  ...
  \caption{Cupping artifact: images}
\end{figure}
\begin{figure}[H]
  \centering
  ...
  \caption{Cupping artifact: intensity profile}
\end{figure}
\paragraph{Analyse}
...
\[ \lambda = \int_{E}^{} b(E) e^{\Pv \mub(E)}  dE\]  
où $\lambda$ est le paramètre de la loi de Poisson qui donne le compte de photons en un pixel du détecteur, b(E) est la fonction donnant le nombre de photons de la source d'énergie comprise entre E et E+dE, et $\Pv$ la matrice de projection. Pour plus d'explications sur la matrice de projection, voir \ref{MatriceProjection} et \ref{MatriceProjection2}.     
\paragraph{Loi somme pour une approximation de spectre discret}	
En pratique, le spectre sera échantillonné en k valeurs d'intensité $b \alpha_k$ avec $\sum_{k}^{} \alpha_k = 1$. L'expression de l'absorption devient donc: 
...
Si l'on gardait l'expression précédente pour la formation des données, on aurait un problème d'optimisation où il faudrait estimer k vecteurs $\mub_k$, ce qui est beaucoup trop. Heureusement DeMan a établi empiriquement une formule qui donne les $\mu_k$ à chaque énergie d'indice k en fonction d'une énergie de référence $\mu_{70}$: l'absorption $\mu$ à 70keV. Ainsi le problème d'estimation a de nouveau autant d'inconnues que précédemment: on cherche uniquement à estimer $\mu_{70}$.
DeMan a obtenu ce résultat en décomposant l'atténuation $\mu(E)$ en deux parties: une partie qui tient compte de l'effet Compton et l'autre de l'effet photoélectrique (deux effets physiques responsables de l'atténuation des rayons X). 
\[ \mu_k = \Phi_k \phi(\mu_{70}) + \Theta_k \theta(\mu_{70}) \]
où $\Phi_k$ et $\Theta_k$ sont des constantes dépendantes uniquement de l'énergie k, et $\phi$ et $\theta$ des fonctions de $\mu$ indépendantes de l'énergie k, qu'on va détailler ci-dessous.
\paragraph{Modèle pour phi et theta: interpolation avec les matériaux}        
        Citer De Man      
Pour obtenir l'expression des fonctions $\phi$ et $\theta$, De Man a calculé leurs valeurs pour de nombreux matériaux (métal, aluminium, eau, os ... à préciser). De Man a ensuite supposé que l'on pouvait interpoler cette fonction entre les points. Il existe d'ailleurs plusieurs méthodes d'interpolation: De Man utilise une méthode linéaire, mais nous verrons par la suite que l'on peut implémenter dans Matlab une fonction qui interpolera entre les points par une hyperbole.
Ci dessous l'image contenant un ensemble de matériaux considérés ainsi que la courbe interpolatrice. 
-----   image à insérer -------
 %ss ss ss ss        
...
On rappelle que le critère à évaluer est le suivant: (cf...)
\[ f ( \mub ) = -\ln( P(\Nv | \mub ) ) \]
avec $f$: la fonction à minimiser (critère), $\mub$ l'image estimée, $\Nv$ le vecteur des comptes de photons observés, qui suit pour chaque pixel une loi de paramètre $\lambda$ qui se calcule grâce à la loi de Beer lambert en polychromatique, comme suit: \\
\[ \lambda = b \sum_{k}^{}  \alpha_k e^{\Pv \mub_k} \]  
...
%ss ss ss ss
 ...
 \subsubsection{Expression de son gradient}
%sSssssssssSssssSssSsSsSsSsS
\section{Implémentation du modèle polychromatique sous Matlab}
%sSssssssssSssssSssSsSsSsSsS
%ss ss ss ss

\subsection{Introduction}
On cherche à modéliser la source pour le critère polychromatique: la source intervient en effet dans l'expression des $\alpha_k$ et de $b$. On expose ici le modèle de réechantillonnage de la source développé par Benoît Hamelin. On cherche à réechantilloner la source pour donner de la flexibilité au modèle: si les calculs sont trop longs on pourra choisir un pas d'échantillonnage plus large.
\subsection{Réechantillonage de la source}
\subsubsection{Format initial de la source}
Le fichier source utilisé par XCAT se présente sous la forme d'un tableau à deux colonnes: une colonne des énergies et une colonne des intensités correspondantes à chaque énergie, avec un pas constant. Les intensités sont en compte de photons par cm^2 et par seconde ( $ \texttt{quanta}/cm^2*s$). On considèrera donc que ce format est le format de base d'une source avant réechantillonnage. Les intensités ne prennent pas en compte l'échantillonnage: les coefficients $\alpha_k*b$ eux en revanche portent en eux le poids de l'échantillonage. On verra ci-dessous dans l'opération de réechantillonage comment obtenir ces coefficients $\alpha_k*b$ avec une discrétisation au pas voulu.
\subsubsection{Décomposition de la source en deux parties}
La source initiale est constituée de deux parties, une partie "lisse", croissante puis décroissante ("triangulaire"), et une partie discontinue,  des pics à deux fréquences caractéristiques. L'idée de Benoît Hamelin pour le réechantillonage du spectre est de rééchantillonner séparément ces deux parties afin de conserver le plus d'informations sur la source (contenu haute fréquence et contenu basse fréquence).\\
...
Pour réechantillonner cette partie, Benoît Hamelin a créé une fonction Matlab \texttt{ge\_model\_poly2line} qui interpole le vecteur source partie "lisse" précédent. On n'a ensuite plus qu'à préciser les nouvelles valeurs d'énergie dont on cherche les intensités, et former ainsi un nouveau vecteur source lisse réechantillonné.
\paragraph{Interpolation de la partie triangulaire, "lisse"}
La fonction d'interpolation de la partie lisse \texttt{ge\_model\_poly2line} développée par Benoît Hamelin est constituée d'un polynôme pour approximer la première partie de la courbe, et d'une droite pour approximer la fin de la courbe. Le degré du polynôme ainsi que l'intensité de bascule entre la partie polynômiale et la partie linéaire sont automatiquement calculés de manière à minimiser l'erreur quadratique avec les données (le vecteur contenant la partie triangulaire "lisse").
\paragraph{Réechantillonage de la partie triangulaire}
Une fois la partie triangulaire interpolée, on peut réechantillonner cette partie: il suffit de préciser le nombre d'échantillons que l'on souhaite, et les intensités correspondantes seront calculées grâce à la fonction d'interpolation \texttt{ge\_model\_poly2line}. Puisque la source réechantillonnée en sortie est en réalité les $\alpha_k *b $, ce vecteur doit aussi prendre en compte le pas d'échantillonage. Pour le trouver on prend le pas de la source initiale, le nouveau pas, et on applique la règle de trois aux intensités.
\subsubsection{Réechantillonage des pics}
Reste maintenant à réechantillonner les pics. On rappelle que l'on a deux ensembles de points "pics" correspondants au voisinage des deux fréquences caractéristiques. 
Benoît Hamelin transforme ces deux ensembles en deux valeurs: une pour chaque pic, qui est la somme de l'ensemble des points contenus dans le pic correspondant (et j'ai corrigé me semble-t-il une erreur en prenant soin de multiplier par la fréquence d'échantillonnage), de la manière suivante: 
%%%%%%%%%%%%%%%%%%%%%
% partie modifiee par moi William pour tenir compte d'un echantillonage differents possible de la source
\lstinputlisting{characteristic_emission_spikes.m}
% EOF
\subsubsection{Résumé}
On a résumé ci-dessous les sources avant et après réechantillonnage pour un échantilonnage de 20 pour la partie lisse, avec comme fichier source initial celui utilisé par XCAT.
\begin{figure}[H]
  \centering
  \subfloat[source initiale]{\label{fig:contour-b}\includegraphics[scale=0.45]{source.eps}}
  \hspace{5pt}
  \subfloat[source réechantillonnée]{\label{fig:contour-c}\includegraphics[scale=0.45]{sourceresampled.eps}}
  \caption{source initiale et source réechantillonnée}
\end{figure}
On remarquera que les valeurs ne sont pas les mêmes. Cela est normal car à gauche il s'agit des valeurs brutes d'intensité de la source à chaque énergie, alors qu'à droite il s'agit de la source réechantillonnée avec un intervalle de 5 keV mais dont chaque valeur d'intensité porte en elle le poids de l'échantillonnage: chaque valeur est donc 5 fois plus grande que dans la figure de gauche.
...
\end{figure}
 ...
 %ss ss ss ss        
   %ss ss ss ss   
	 ...
	 \subsection{Formulation Matlab du critère et de son gradient}
	 \subsubsection{Choix des paramètres à rentrer en argument du critère}
		\begin{itemize}
		\item Matrice de projection
		\item Matériaux considérés, matériaux par défaut
		\item Source considérée
		\item Echantillonnage de la source désiré
 		\end{itemize}
		
\chapter{Converger vers une bonne image}
\section{Préconditionnement}
\section{Régularisations successives}		
...
$\mub$: coefficients d'atténuation \\
$\yb_{i}$: distribution de Poisson indépendant de i, de paramètre $\hat{y}_i$ \\
...
\begin{equation}
J = \sum_{i}^{} (\hat{y}_i - y_i \log{y}_i) \\
 = b \sum_{i}^{} (e^{-\Pv_{i, \cdot} \mub} - \cancel{\frac{y_i}{b} \log b} + \frac{y_i}{b} \Pv_{i, \cdot}) \\
  = b \sum_{i}^{} + \frac{y_i}{b} \Pv_{i, \cdot}
\end{equation}
\section{Cadre polychromatique}
Maintenant: $ \hat{y}_i = b \sum_{k=1}^{K} \alpha_k e^{-\sum_{j}^{} a_{ij} \mu_{jk}} $ \\
$k$ : indice d'énergie ou de fréquence du rayonnement
On utilise la décomposition d'Avarez-Macovski, paramétrisée adéquatement, pour représenter $\mu$: 
\[ \mu_k = b \sum_{k=1}^{K} \phi \Phi_k + \theta \Theta_k \]
...
\[ \mu = \sum_{k=1}^{K} \phi(\mu_{70}) \Phi_k + \theta(\mu_{70}) \Theta_k \]
...
\end{equation}
Calcul de $ \frac{\partial \tilde{y}-i}{\partial \mu} $:
...
\end{equation}
et $ \tilde{y_i} = \sum_{k=1}{K} \tilde{y}_{i, k} $ 
...
\section{Calcul de la log-vraisemblance}
Rappels: On rappelle que l'on repart du critère polychromatique suivant: \\
...
\end{equation}
On fait alors apparaître $e^{-y_i}$.
\begin{equation}
J(\mub) = \sum_{i}^{} \{ \sum_{k}^{} \alpha_k e^{-y_i}e^{ y_i + \texttt{Proj(i,k)} } - \frac{y_i}{b} \log(\sum_{k}^{} \alpha_k e^{-y_i}e^{ y_i +  \texttt{Proj(i,k)}}) \}
\end{equation}
On pose: $ h_{i,k} = y_i -\Phi_k \Pv_{i, \cdot}\phi(\mub) - \Theta_k \Pv_{i, \cdot}\theta(\mub) $\\
L'équation devient alors : $   J(\mub) = \sum_{i}^{} \{ \sum_{k}^{} \alpha_k e^{-y_i}e^{ h_{i,k} } - \frac{y_i}{b} \log(\sum_{k}^{} \alpha_k e^{-y_i}e^{h_{i,k} )})  $\\
On effectue un développement limité autour de $y_i$.\\
On a : $e^{ h_{i,k} } = 1+h_{i,k} + \frac{h_{i,k}^2}{2} $.\\
d'où: 
\[   J(\mub) = \sum_{i}^{} \{ \sum_{k}^{} \alpha_k e^{-y_i}(1+h_{i,k} + \frac{h_{i,k}^2}{2}) - \frac{y_i}{b} \log(\sum_{k}^{} \alpha_k e^{-y_i}(1+h_{i,k} + \frac{h_{i,k}^2}{2})   ) \]
\[ J(\mub) = \sum_{i}^{} \{ \sum_{k}^{} \alpha_k e^{-y_i}(1+h_{i,k} + \frac{h_{i,k}^2}{2}) - \frac{y_i}{b} \underbrace{\log(\sum_{k}^{} \alpha_k (1+h_{i,k} + \frac{h_{i,k}^2}{2})}_{A}   ) -\frac{y_i}{b} \log(e^{-y_i})  \]
\[ A =  \log(\sum_{k}^{} \alpha_k + \sum_{k}^{} \alpha_k ( h_{i,k} +\frac{h_{i,k}^2}{2})  \]
\[ A =  \log(1 + \sum_{k}^{} \alpha_k ( h_{i,k} +\frac{h_{i,k}^2}{2})  \]
En poursuivant le développement limité: ---- à justifier c'est pas tant correct ------
\[ A =  \sum_{k}^{} \alpha_k ( h_{i,k} +\frac{h_{i,k}^2}{2} -\frac{1}{2} (\sum_{k}^{} \alpha_k h_{i,k})^2)  \] \\
En se débarassant des termes qui ne dépendent pas de \mub, on obtient la nouvelle expression du critère approximé, avec $ y_i = \log(\frac{N_0}{N_i})$.
\[ J(\mub) = \sum_{i}{} (\sum_{k}{} \alpha_k ( y_i - \Phi_k \Pv_{i, \cdot}\phi(\mub) - \Theta_k \Pv_{i, \cdot}\theta(\mub) ) )\}\]
\[J(\mub) = \frac{N_0}{2} \sum_{i}{} e^{-y_i} ( y_i - (\sum_{k}{} \alpha_k \Phi_k) \Pv_{i,\cdot} \Phi(\mub) - (\sum_{k}{} \alpha_k \Theta_k) \Pv_{i,\cdot} \Theta(\mub))^2 \]
De manière vectorielle, la dernière expression peut se simplifier en:
\[   J(\mub) = \texttt{Sum}(\frac{\Nv}{2} .* (\Sv)\hat{ .} 2)  \]
avec 
\[  \Sv = \yb - \pb \Pv \Phi(\mub) - \tb \Pv \Theta(\mub)   \]
avec
...
On pose :
\[   J_i(\mub) = \frac{\Nv_i}{2} * (\Sv_i)^2  \]
Alors: 
\[ \frac{\partial J_i(\mub)}{\partial \mu_j} = \Nv_i * \frac{\partial \Sv_i}{\partial \mu_j} * \Sv_i \]
En remplaçant:
\[ \frac{\partial J_i(\mub)}{\partial \mu_j} = \Nv_i * (-\pb \Pv_{i,j} \Phi'(\mu_j) - \tb \Pv_{i,j} \Theta'(\mu_j))* (\yb - \pb \Pv_{i,\cdot} \Phi(\mub) - \tb \Pv_{i,\cdot} \Theta(\mub) ) \]
On peut réordonner les termes car on travaille ici avec des scalaires:
 \[ \frac{\partial J_i(\mub)}{\partial \mu_j} = (- \pb \Phi'(\mu_j) - \tb \Theta'(\mu_j))*  \Pv_{i,j}(\yb - \pb \Pv_{i,\cdot} \Phi(\mub) - \tb \Pv_{i,\cdot} \Theta(\mub) ) * \Nv_i \]
 Ceci est une forme qui peut s'exprimer plus simplement de manière vectorielle:
 \[ \nabla_\mu J(\mub)= (- \pb \Phi'(\mu) - \tb \Theta'(\mu)) .*  \Pv^T(\yb - \pb \Pv \Phi(\mub) - \tb \Pv\Theta(\mub) ) .* \Nv \]
soit avec les notations précédentes:
\[ \nabla_\mu J(\mub)= (- \pb \Phi'(\mu) - \tb \Theta'(\mu)) .*  \Pv^T.(\Sv .* \Nv) \]
Ce qui s'implémente facilement dans Matlab:
\begin{itemize}
\item On calcule $  \Nv $
\item On calcule $ \Pv^T.(\Sv .* \Nv)  $ avec la fonction \texttt{A.RProject(S*N)}
