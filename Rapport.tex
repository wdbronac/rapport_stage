...
Dans ces test nous changeons la tolérance demandée sur le gradient du critère. Nous verrons que pour une tolérance de $10^{-5}$, les images sont assez différentes, alors qu'il y a bien une image finale, que l'on peut obtenir si l'on force plus la convergence en diminuant la tolérance jusqu'à $10^{-8}$. Les paramètres de la simulation sont les mêmes qu'en \ref{Psarametres}, et avec un nombre de rayons par détecteurs de 8.	
%%%%%%
...
DeMan a obtenu ce résultat en décomposant l'atténuation $\mub_k$ en deux parties (décomposition d'Alvarez-Makcovski): une partie qui tient compte de l'effet Compton et l'autre de l'effet photoélectrique (deux effets physiques responsables de l'atténuation des rayons X).
 \[ \mub_k = \underbrace{\Phi_k \phi}_{photoélectrique} + \underbrace{\Theta_k \theta}_{Compton} \]
...
\[ \mub_k = \underbrace{\Phi_k \phi(\mub_{70})}_{photoélectrique} + \underbrace{\Theta_k \theta(\mub_{70})}_{Compton} \]
...
\paragraph{ré-échantillonnage de la partie triangulaire}
...
\[ J(\mub) = \sum_{i}{} (\sum_{k}{} \alpha_k ( s_i - \Phi_k \Pv_{i, \cdot}\phi(\mub) - \Theta_k \Pv_{i, \cdot}\theta(\mub) ) )\}\]
...
\[   J(\mub) = \texttt{Sum}(\frac{\yb}{2} .* (\Sv)\hat{ .} 2)  \]
avec 
\[  \Sv = \sb - \p \Pv \Phi(\mub) - \t \Pv \Theta(\mub)   \]
avec
\[  \pb =  \sum_{k}{} \alpha_k \Phi_k  \]
\[  \tb =  \sum_{k}{} \alpha_k \Theta_k   \]
\section{Calcul du gradient de la neg-log-vraisemblance}
